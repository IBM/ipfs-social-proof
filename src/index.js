...
class IpfsIdentity {
  ...
  async getAllProofs () {
    await this._storageDB.get(DB_PROOFS_KEY).
      then((result) => {
  ...
  async _saveProof (hash, content) {
    content.ipfsHash = hash
    let proofs = this.getAllProofs()

    await this._storageDB.put(DB_PROOFS_KEY, JSON.stringify(content)).
      then((result) => {
        return JSON.parse(result)
      }).
      catch((err) => {
  ...
  saveProofToDb (hash, content, callback) {
    const storedData = {
      ipfsPath: hash,
      content: content
    }
    ...
    this._storageDB.put(
      DB_PROOFS_KEY,
      JSON.stringify(storedData), (err) => {
        if (err) {
          console.error(err)
        }
        console.info('Proof saved locally')
      })
  ...
  getProof (hash) {
    ...
    proof._hash = hash
    return JSON.sringify(proof)
  }

  async getProofData () {
    // this populates the in-memory _proofData property
    const that = this
    await this.proofDB.createReadStream()
  ...
  getMultihashForStringContent (stringContent, callback) {
    ...
    multihashing(buf, 'sha2-256', (err, mh) => {
      if (err) {
        return callback(err, null)
      }
